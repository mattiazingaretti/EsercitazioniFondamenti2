...
linked_list * graph_get_incident_edges(graph * g, graph_node * n) {
	if (g && n)
		...
		printf("ERROR - invocation with NULL in graph_get_incident_edges\n");
		return NULL;
	}
}

graph_node * graph_add_node(graph * g, void* obj) {
	if (g && obj) {
...
const void* graph_get_node_name(graph_node* n) {
	if (n)
		return n->object;
	else {
		printf("ERROR - invocation with NULL in graph_get_node_name, returning NULL as default value\n");
		return NULL;
	}
}
...
void graph_print(graph* g) {
	if (g) {
		...
		while (linked_list_iterator_hasnext(it)) {
			...
			graph_edge* edge = linked_list_iterator_getvalue(it2);
			while (linked_list_iterator_hasnext(it2)) {
				printf("%p:%f ", edge->n2 == cur ? edge->n1->object: edge->n2->object, edge->weight);
