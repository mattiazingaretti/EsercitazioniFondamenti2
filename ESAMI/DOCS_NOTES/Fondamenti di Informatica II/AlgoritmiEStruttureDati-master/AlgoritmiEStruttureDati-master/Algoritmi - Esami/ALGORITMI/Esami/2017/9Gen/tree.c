...
void cerca_percorso(node* nodo, char* s, char* buf){
    if(nodo->nextSibling != NULL) cerca_percorso(nodo->nextSibling, s, buf);
    char* buffer = aggiorna_percorso(buf, nodo->etichetta);
    if(strcmp(nodo->etichetta, s) == 0){
        printf("%s\n", buffer);
    }
    if(nodo->firstChild != NULL) cerca_percorso(nodo->firstChild, s, buffer);
}
